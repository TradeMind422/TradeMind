...
async function callAI(u,s){try{const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,system:s,messages:[{role:"user",content:u}]})});const d=await r.json();return JSON.parse(d.content.map(i=>i.text||"").join("").replace(/```json|```/g,"").trim())}catch{return null}}
...
function Btn({ch,onClick,style={},dis=false}){return <button onClick={onClick} disabled={dis} className="btn" style={{width:"100%",padding:"15px",borderRadius:12,border:"none",background:dis?"#333":"linear-gradient(135deg,#F5A623,#E8803A)",color:dis?"#666":"#000",fontSize:15,fontWeight:800,cursor:dis?"not-allowed":"pointer",fontFamily:"inherit",marginTop:12,...style}}>{ch}</button>}

function JCard({j}){
  const d=j.created_at?new Date(j.created_at).toLocaleDateString("en-GB",{day:"numeric",month:"short"}):j.date||"Today"
  return <div style={{background:G.bg2,border:`1px solid ${G.border}`,borderRadius:14,padding:"14px 16px",marginBottom:10}}>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
      ...
      <div style={{textAlign:"right"}}>
        <p style={{fontSize:15,fontWeight:800,color:j.pnl?.includes("-")?G.red:G.green,margin:"0 0 2px"}}>{j.pnl}</p>
...
function BottomNav({screen,setScreen}){
  ...
  return <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:"rgba(7,9,14,0.97)",borderTop:`1px solid ${G.border}`,display:"flex",justifyContent:"space-around",padding:"10px 0 18px",zIndex:100}}>
    {T.map(t=><button key={t.id} onClick={()=>setScreen(t.id)} style={{display:"flex",flexDirection:"column",alignItems:"center",background:"none",border:"none",cursor:"pointer",padding:"4px 10px",fontFamily:"inherit",position:"relative"}}>
...
function Splash(){return <div style={{minHeight:"100vh",background:G.bg,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
...
function Landing({onLogin,onSignup}){return <div style={{minHeight:"100vh",background:G.bg,overflowY:"auto"}}>
  <div style={{padding:"48px 22px 60px",maxWidth:480,margin:"0 auto"}} className="fi">
    ...
    <div style={{display:"flex",justifyContent:"space-around",background:G.bg2,border:`1px solid ${G.border}`,borderRadius:16,padding:"18px 10px",marginBottom:24}}>
      {[{v:"2,400+",l:"Active Traders"},{v:"94%",l:"Improved Win Rate"},{v:"$1.2M",l:"Losses Prevented"}].map(s=><div key={s.l} style={{textAlign:"center"}}><p style={{fontSize:22,fontWeight:800,color:G.gold,margin:"0 0 2px"}}>{s.v}</p><p style={{fontSize:10,color:G.muted,margin:0}}>{s.l}</p></div>)}
    </div>
...
function AuthWrap({title,sub,onBack,children}){return <div style={{minHeight:"100vh",background:G.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:"24px 20px"}}>
  <div className="fi" style={{background:G.bg2,border:`1px solid ${G.border}`,borderRadius:20,padding:"32px 24px",width:"100%",maxWidth:400}}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:24}}><span style={{fontSize:26}}>📈</span><span style={{fontSize:20,fontWeight:800,background:"linear-gradient(135deg,#F5D88A,#E8803A)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>TradeMind</span></div>
...
function FInput({label,type,value,onChange,placeholder}){return <div style={{marginBottom:16}}>
...
function Login({onAuth,onSwitch,onBack}){
  ...
  return <AuthWrap title="Welcome back" sub="Sign in to your account" onBack={onBack}>
...
function Signup({onAuth,onSwitch,onBack}){
  ...
  return <AuthWrap title="Create account" sub="Join 2,400+ traders" onBack={onBack}>
...
function Home({user,setScreen,journals,todayCount}){
  ...
  const pnl=journals.reduce((a,j)=>{const n=parseFloat((j.pnl||"0").replace(/[^0-9.-]/g,""));return a+((j.pnl||"").includes("-")?-n:n)},0)
  return <div style={{padding:"28px 18px 16px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:16}}>
      {[{v:`${wr}%`,l:"Win Rate",c:G.green},{v:pnl>=0?`+$${Math.round(pnl)}`:`-$${Math.abs(Math.round(pnl))}`,l:"P&L",c:pnl>=0?G.green:G.red},{v:`${journals.length}🔥`,l:"Streak",c:G.gold},{v:journals.length,l:"Journals",c:G.blue}].map(s=><div key={s.l} style={{background:G.bg2,border:`1px solid ${G.border}`,borderRadius:12,padding:"12px 6px",textAlign:"center"}}><p style={{fontSize:14,fontWeight:800,color:s.c,margin:"0 0 2px"}}>{s.v}</p><p style={{fontSize:9,color:G.muted,margin:0}}>{s.l}</p></div>)}
    </div>
    <button onClick={()=>setScreen("journal")} className="btn" style={{width:"100%",background:"rgba(245,166,35,0.08)",border:"1px solid rgba(245,166,35,0.25)",borderRadius:16,padding:18,display:"flex",alignItems:"center",gap:14,cursor:"pointer",marginBottom:16,textAlign:"left"}}>
      ...
      <div style={{flex:1}}><p style={{fontSize:17,fontWeight:700,color:G.text,margin:"0 0 2px"}}>Log a Trade</p><p style={{fontSize:11,color:G.muted,margin:0}}>{user?.isPremium?"Unlimited · PRO":`${todayCount}/3 today`}</p></div>
      <span style={{color:G.gold,fontSize:20}}>→</span>
    </button>
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:24}}>
      {[{i:"🧠",l:"Insights",s:"insights"},{i:"📅",l:"History",s:"history"},{i:user?.isPremium?"👑":"🔒",l:user?.isPremium?"PRO":"Upgrade",s:"pricing"}].map(q=><button key={q.l} onClick={()=>setScreen(q.s)} style={{background:G.bg2,border:`1px solid ${G.border}`,borderRadius:14,padding:"16px 8px",display:"flex",flexDirection:"column",alignItems:"center",gap:6,cursor:"pointer",fontFamily:"inherit"}}><span style={{fontSize:22}}>{q.i}</span><p style={{fontSize:11,color:G.muted,margin:0,fontWeight:600}}>{q.l}</p></button>)}
    </div>
    ...
    {!user?.isPremium&&<div onClick={()=>setScreen("pricing")} style={{display:"flex",alignItems:"center",gap:12,background:"rgba(245,166,35,0.05)",border:"1px solid rgba(245,166,35,0.18)",borderRadius:14,padding:"14px 16px",marginTop:12,cursor:"pointer"}}><div style={{flex:1}}><p style={{fontSize:14,fontWeight:700,color:G.text,margin:"0 0 2px"}}>🧠 Unlock Psychology Insights</p><p style={{fontSize:11,color:G.muted,margin:0}}>Detect patterns. Fix mindset. Grow account.</p></div><span style={{color:G.gold}}>→</span></div>}
  </div>
}
...
function Journal({setScreen,user,setJournals,todayCount,setTodayCount}){
  ...
  function startRec(){
    ...
    if(!SR){setVoice("Not supported. Type below.");return}
    const r=new SR();r.continuous=true;r.interimResults=true
    r.onresult=e=>{let t="";for(let i=0;i<e.results.length;i++)t+=e.results[i][0].transcript;setVoice(t)}
  ...
  async function analyze(){
    ...
    setStep("processing")
    const r=await callAI(`Trade:${pair}|Result:${result}|P&L:${pnl}\nJournal:${voice}`,`Elite trading psychology coach. JSON only:\n{"summary":"2 sentences","emotion":"one word","patternWarning":null or "text","tip":"tip","score":1-10}`)
    const res=r||{summary:"Journal saved.",emotion:"Noted",patternWarning:null,tip:"Keep journaling.",score:7}
    ...
    setJournals(p=>[saved||{id:Date.now(),pair,result,pnl,emotion:res.emotion,summary:res.summary,pattern:res.patternWarning,score:res.score},...p])
    setTodayCount(c=>c+1);setAiRes(res);setStep("done")
  }
  if(blocked)return <div style={{padding:"28px 18px"}}>
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",minHeight:"60vh",justifyContent:"center",gap:16}}>
      ...
      <p style={{fontSize:14,color:G.muted,margin:0}}>Free users log 3 trades/day.</p>
      <Btn ch="Upgrade to PRO 👑" onClick={()=>setScreen("pricing")}/>
    </div>
  </div>
  return <div style={{padding:"28px 18px"}} className="fi">
    <button onClick={()=>setScreen("home")} style={{background:"none",border:"none",color:G.muted,fontSize:13,cursor:"pointer",padding:0,marginBottom:18}}>← Back</button>
    {step==="checkin"&&<>
      ...
      {user?.isPremium?[{k:"sleep",q:"How did you sleep?",o:["Great","Okay","Badly"]},{k:"mood",q:"Current mindset?",o:["Sharp","Neutral","Off"]},{k:"revenge",q:"Chasing losses?",o:["No","Maybe","Yes"]}].map(q=><div key={q.k} style={{marginBottom:20}}>
        <p style={{fontSize:14,fontWeight:600,color:"#C8C0B4",margin:"0 0 10px"}}>{q.q}</p>
        <div style={{display:"flex",gap:8}}>{q.o.map(o=><button key={o} onClick={()=>setCheckin(p=>({...p,[q.k]:o}))} style={{padding:"9px 18px",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"inherit",background:checkin[q.k]===o?G.gold:"rgba(255,255,255,0.05)",color:checkin[q.k]===o?"#000":"#C8C0B4",border:`1px solid ${checkin[q.k]===o?G.gold:"rgba(255,255,255,0.1)"}`}}>{o}</button>)}</div>
      </div>):<div style={{display:"flex",alignItems:"center",gap:10,background:G.bg2,border:`1px solid ${G.border}`,borderRadius:12,padding:16,marginBottom:20,color:"#333",fontSize:13}}><span>🔒</span><p style={{margin:0}}>Pre-trade check-in is PRO only</p></div>}
    ...
    {step==="record"&&<>
      ...
      <div style={{display:"flex",gap:12,marginBottom:14}}>
        <div style={{flex:1}}>
          ...
          <select value={pair} onChange={e=>setPair(e.target.value)} style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${G.border}`,borderRadius:10,padding:"11px 12px",color:G.text,fontSize:14,fontFamily:"inherit",outline:"none"}}><option value="">Select</option>{PAIRS.map(p=><option key={p}>{p}</option>)}</select>
        </div>
        <div style={{flex:1}}>
          <p style={{fontSize:10,letterSpacing:"2px",color:G.muted,margin:"0 0 8px",fontWeight:700}}>P&L</p>
          <input value={pnl} onChange={e=>setPnl(e.target.value)} placeholder="+$240" style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${G.border}`,borderRadius:10,padding:"11px 12px",color:G.text,fontSize:14,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}}/>
        </div>
      </div>
      <p style={{fontSize:10,letterSpacing:"2px",color:G.muted,margin:"0 0 8px",fontWeight:700}}>RESULT</p>
      <div style={{display:"flex",gap:8,marginBottom:18}}>{["Win","Loss","Breakeven"].map(r=><button key={r} onClick={()=>setResult(r)} style={{flex:1,padding:"11px 4px",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",background:result===r?(r==="Win"?G.green:r==="Loss"?G.red:G.gold):"rgba(255,255,255,0.05)",border:`1px solid ${result===r?(r==="Win"?G.green:r==="Loss"?G.red:G.gold):"rgba(255,255,255,0.08)"}`}}>{r}</button>)}</div>
      <button onClick={recording?stopRec:startRec} style={{width:"100%",padding:14,borderRadius:12,cursor:"pointer",fontSize:14,fontWeight:700,fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:12,background:recording?"rgba(224,92,92,0.1)":"rgba(245,166,35,0.07)",border:`1px solid ${recording?G.red:"rgba(245,166,35,0.3)"}`}}>
        <span style={{fontSize:20}}>{recording?"⏹":"🎙️"}</span><span>{recording?"Stop Recording":"Start Voice Journal"}</span>
      </button>
      <textarea value={voice} onChange={e=>setVoice(e.target.value)} placeholder="Or type here — what happened? How did you feel?" rows={5} style={{width:"100%",background:"rgba(255,255,255,0.04)",border:`1px solid ${G.border}`,borderRadius:10,padding:12,color:G.text,fontSize:14,resize:"none",fontFamily:"inherit",boxSizing:"border-box",marginBottom:4}}/>
      <Btn ch="Analyze with AI 🧠" onClick={analyze} dis={!voice||!pair||!result}/>
    </>}
    {step==="processing"&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"60vh",gap:18}}>
      <div style={{width:80,height:80,borderRadius:"50%",background:"rgba(245,166,35,0.08)",border:"1px solid rgba(245,166,35,0.2)",display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
        <div className="os"/><span style={{fontSize:32,position:"absolute"}}>🧠</span>
      </div>
      <p style={{fontSize:20,fontWeight:700,color:G.text,margin:0}}>Analyzing psychology...</p>
      {[80,65,90].map((w,i)=><div key={i} className="sh" style={{height:12,borderRadius:6,background:"rgba(255,255,255,0.05)",width:`${w}%`}}/>)}
    </div>}
    {step==="done"&&aiRes&&<div className="fi">
      ...
      <div style={{display:"flex",alignItems:"center",gap:20,background:"rgba(245,166,35,0.07)",border:"1px solid rgba(245,166,35,0.2)",borderRadius:16,padding:"18px 20px",marginBottom:16}}>
        <div style={{display:"flex",alignItems:"baseline",gap:2}}><p style={{fontSize:52,fontWeight:800,color:G.gold,lineHeight:1,margin:0}}>{aiRes.score}</p><p style={{fontSize:18,color:G.muted,margin:0}}>/10</p></div>
        <div><p style={{fontSize:9,letterSpacing:"2px",color:G.muted,margin:"0 0 4px",fontWeight:700}}>MINDSET SCORE</p><p style={{fontSize:20,fontWeight:700,color:G.text,margin:0}}>{aiRes.emotion}</p></div>
      </div>
      <div style={{background:G.bg2,border:`1px solid #555`,borderRadius:12,padding:"14px 16px",marginBottom:12}}><p style={{fontSize:10,letterSpacing:"2px",fontWeight:700,color:"#555",margin:"0 0 6px"}}>📝 SUMMARY</p><p style={{fontSize:13,color:"#B0A89C",lineHeight:1.65,margin:0}}>{aiRes.summary}</p></div>
      {aiRes.patternWarning&&<div style={{background:G.bg2,border:`1px solid ${G.red}`,borderRadius:12,padding:"14px 16px",marginBottom:12}}><p style={{fontSize:10,letterSpacing:"2px",fontWeight:700,color:G.red,margin:"0 0 6px"}}>⚠️ PATTERN</p><p style={{fontSize:13,color:"#B0A89C",lineHeight:1.65,margin:0}}>{aiRes.patternWarning}</p></div>}
      <div style={{background:G.bg2,border:`1px solid ${G.green}`,borderRadius:12,padding:"14px 16px",marginBottom:12}}><p style={{fontSize:10,letterSpacing:"2px",fontWeight:700,color:G.green,margin:"0 0 6px"}}>💡 TIP</p><p style={{fontSize:13,color:"#B0A89C",lineHeight:1.65,margin:0}}>{aiRes.tip}</p></div>
...
function History({setScreen,user,journals}){
  ...
  return <div style={{padding:"28px 18px"}} className="fi">
    ...
    <div style={{display:"flex",gap:8,marginBottom:16}}>{["All","Win","Loss","Flagged"].map(x=><button key={x} onClick={()=>setFilter(x)} style={{padding:"7px 14px",borderRadius:20,border:"none",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",background:filter===x?G.gold:"rgba(255,255,255,0.05)",color:filter===x?"#000":G.muted}}>{x}</button>)}</div>
...
function SkillItem({skill}){
  ...
  return <div style={{background:G.bg2,border:`1px solid ${G.border}`,borderRadius:12,padding:14}}>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
...
function Insights({setScreen,user}){
  ...
  if(!user?.isPremium)return <div style={{padding:"28px 18px"}} className="fi">
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",padding:"40px 20px"}}>
      ...
      <p style={{fontSize:14,color:G.muted,lineHeight:1.7,margin:"0 0 24px"}}>Advanced AI psychology analysis used by funded traders.</p>
      <Btn ch="Unlock Insights — $19.99/mo 👑" onClick={()=>setScreen("pricing")}/>
    </div>
  </div>
  return <div className="fi">
    ...
    {v&&<div style={{background:"rgba(245,166,35,0.04)",border:"1px solid rgba(245,166,35,0.15)",borderRadius:14,padding:"16px 18px",margin:"0 18px 16px"}}>
      <p style={{fontSize:9,letterSpacing:"2.5px",color:G.gold,margin:"0 0 6px",fontWeight:700}}>🤖 AI VERDICT</p>
    ...
    <div style={{display:"flex",borderBottom:`1px solid ${G.border}`,overflowX:"auto"}}>
      {["overview","skills"].map(t=><button key={t} onClick={()=>setTab(t)} style={{background:"none",border:"none",borderBottom:tab===t?`2px solid ${G.gold}`:"2px solid transparent",color:tab===t?G.gold:"#333",fontSize:12,fontWeight:700,padding:"12px 18px",cursor:"pointer",fontFamily:"inherit"}}>{t.charAt(0).toUpperCase()+t.slice(1)}</button>)}
    </div>
    <div style={{padding:"16px 18px 24px"}}>
      {tab==="overview"&&v&&<div className="fi">
        <div style={{display:"flex",alignItems:"center",gap:18,background:G.bg2,border:`1px solid ${G.border}`,borderRadius:16,padding:18,marginBottom:16}}>
          ...
          <div><p style={{fontSize:9,letterSpacing:"2px",color:G.muted,margin:"0 0 4px",fontWeight:700}}>MINDSET SCORE</p><p style={{fontSize:22,fontWeight:800,color:G.text,margin:"0 0 4px"}}>B+ · 72/100</p><p style={{fontSize:12,color:G.muted,margin:0}}>Emotional interference cost ~$620 this week.</p></div>
        </div>
        <div style={{background:"rgba(224,92,92,0.06)",border:"1px solid rgba(224,92,92,0.2)",borderRadius:14,padding:16,marginBottom:12}}><p style={{fontSize:9,letterSpacing:"2px",color:G.red,margin:"0 0 6px",fontWeight:700}}>⚡ CRITICAL FIX</p><p style={{fontSize:13,color:"#B08888",lineHeight:1.65,margin:0}}>{v.critical}</p></div>
        <div style={{border:"1px solid",borderColor:G.green,background:"rgba(76,175,125,0.05)",borderRadius:12,padding:"14px 16px",marginBottom:12}}><p style={{fontSize:9,letterSpacing:"2.5px",fontWeight:800,color:G.green,margin:"0 0 6px"}}>🏆 YOUR EDGE</p><p style={{fontSize:13,color:"#8E8882",lineHeight:1.65,margin:0}}>{v.strength}</p></div>
        <div style={{border:"1px solid",borderColor:G.blue,background:"rgba(107,127,215,0.05)",borderRadius:12,padding:"14px 16px"}}><p style={{fontSize:9,letterSpacing:"2.5px",fontWeight:800,color:G.blue,margin:"0 0 6px"}}>🎯 THIS WEEK</p><p style={{fontSize:13,color:"#8E8882",lineHeight:1.65,margin:0}}>{v.nextWeek}</p></div>
      </div>}
      {tab==="skills"&&<div className="fi" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
...
function Profile({user,setScreen,setUser,onLogout}){
  return <div style={{padding:"28px 18px"}} className="fi">
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:24}}>
      {[{i:"👑",l:user?.isPremium?"Manage Subscription":"Upgrade to PRO",s:"pricing",hi:!user?.isPremium},{i:"🎯",l:"Trading Goals",s:null},{i:"💬",l:"Help & Support",s:null}].map(item=><button key={item.l} onClick={()=>item.s&&setScreen(item.s)} style={{display:"flex",alignItems:"center",gap:14,background:G.bg2,border:`1px solid ${item.hi?"rgba(245,166,35,0.3)":G.border}`,borderRadius:12,padding:"14px 16px",cursor:"pointer",textAlign:"left",fontFamily:"inherit",width:"100%"}}><span style={{fontSize:18}}>{item.i}</span><p style={{flex:1,fontSize:14,margin:0,fontWeight:600,color:item.hi?G.gold:"#C8C0B4"}}>{item.l}</p><span style={{color:"#444"}}>›</span></button>)}
    </div>
...
function Pricing({setScreen,user,setUser}){
  const[loading,setLoading]=useState(false)
  const upgrade=async()=>{setLoading(true);await sb.updatePremium(user.id,user.token);setUser(u=>({...u,isPremium:true}));setLoading(false);setScreen("home")}
  const FR=({t,on,g})=><div style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:8}}><span style={{color:on?(g?G.gold:G.green):"#333",fontSize:13}}>{on?"✓":"✗"}</span><span style={{fontSize:12,color:on?"#C8C0B4":"#3A3A3A",lineHeight:1.4}}>{t}</span></div>
  return <div style={{padding:"28px 18px"}} className="fi">
    <button onClick={()=>setScreen("home")} style={{background:"none",border:"none",color:G.muted,fontSize:13,cursor:"pointer",padding:0,marginBottom:18}}>← Back</button>
    ...
    <div style={{background:G.bg2,border:`1px solid ${G.border}`,borderRadius:16,padding:20,marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><p style={{fontSize:20,fontWeight:800,margin:0,color:G.text}}>Free</p><p style={{fontSize:26,fontWeight:800,margin:0,color:G.text}}>$0<span style={{fontSize:13,color:G.muted}}>/mo</span></p></div>
      {["3 journals/day","7-day history","AI summary"].map(t=><FR key={t} t={t} on/>)}
      {["Pattern detection","Insights","Unlimited history"].map(t=><FR key={t} t={t}/>)}
    ...
    <div style={{background:"rgba(245,166,35,0.03)",border:"1px solid rgba(245,166,35,0.4)",borderRadius:16,padding:20,marginBottom:14,position:"relative"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><p style={{fontSize:20,fontWeight:800,margin:0,color:G.gold}}>PRO 👑</p><p style={{fontSize:26,fontWeight:800,margin:0,color:G.text}}>$19.99<span style={{fontSize:13,color:G.muted}}>/mo</span></p></div>
      {["Unlimited journals","Full history","Pattern detection","Psychology insights","Pre-trade check-in","AI coaching","Priority support"].map(t=><FR key={t} t={t} on g/>)}
      {user?.isPremium?<div style={{textAlign:"center",padding:10,borderRadius:8,background:"rgba(245,166,35,0.1)",color:G.gold,fontSize:12,marginTop:12}}>✓ Active</div>:<Btn ch={loading?"Upgrading...":"Upgrade — $19.99/mo"} onClick={upgrade} dis={loading}/>}
    </div>
    <div style={{background:G.bg2,border:"1px solid rgba(107,127,215,0.35)",borderRadius:16,padding:20,position:"relative"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><p style={{fontSize:20,fontWeight:800,margin:0,color:G.blue}}>Lifetime 🚀</p><p style={{fontSize:26,fontWeight:800,margin:0,color:G.text}}>$149<span style={{fontSize:13,color:G.muted}}> once</span></p></div>
      <Btn ch="Get Lifetime Access" onClick={upgrade} style={{background:"linear-gradient(135deg,#6B7FD7,#4B5FBF)"}} dis={loading}/>
    </div>
  </div>
}
...
export default function App(){
  ...
  useEffect(()=>{if(user?.id&&user?.token){sb.getJournals(user.id,user.token).then(d=>{if(Array.isArray(d))setJournals(d)})}},[user?.id])
  const login=u=>{setUser(u);setAuth("app");setScreen("home")}
  const logout=async()=>{if(user?.token)await sb.signOut(user.token);setUser(null);setJournals([]);setAuth("landing");setScreen("home")}
  ...
  return<div style={{minHeight:"100vh",background:G.bg,fontFamily:"'Sora',sans-serif",color:G.text,maxWidth:480,margin:"0 auto",display:"flex",flexDirection:"column"}}><style>{CSS}</style><div style={{flex:1,overflowY:"auto",paddingBottom:72}}>{screen==="home"&&<Home user={user} setScreen={setScreen} journals={journals} todayCount={todayCount}/>}{screen==="journal"&&<Journal setScreen={setScreen} user={user} setJournals={setJournals} todayCount={todayCount} setTodayCount={setTodayCount}/>}{screen==="history"&&<History setScreen={setScreen} user={user} journals={journals}/>}{screen==="insights"&&<Insights setScreen={setScreen} user={user}/>}{screen==="profile"&&<Profile user={user} setScreen={setScreen} setUser={setUser} onLogout={logout}/>}{screen==="pricing"&&<Pricing setScreen={setScreen} user={user} setUser={setUser}/>}</div><BottomNav screen={screen} setScreen={setScreen}/></div>
}
